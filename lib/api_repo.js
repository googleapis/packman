...
var pbjs_util = require('protobufjs/cli/pbjs/util'); // eslint-disable-line
 ...
 * @param {?function} optDone Optional callback.
 */
 ...
 * @param {string} name The API name.
 * @param {string} version The API version.
 * @param {?function} optDone The optional callback.
 */
 ...
 * @param {?function} optDone The optional callback.
 * @return {function} a function which can be used as a callback.
 */
 ...
 *
 * @param {Array.<string[]>} fileLists - the list of the lists of the .proto
 *   files to be copied. Each item is the list of .proto files calculated by
 *   collectProtoDeps.
 ...
 * @return {function} The function of doing the actual task.
 */
 ...
 * @return {function} A funciton to invoke protoc command.
 */
ApiRepo.prototype._makeProtocFunc = function _makeProtocFunc(opts, language) {
  ...
  function callProtoc(protoPath, done) {
    ...
    if (language === 'go') {
      ...
      if (!opts.buildCommonProtos) {
        var pluginBin = that.depBins[that._getPluginName(
            language, that.overridePlugins)];
 ...
 */
pbjs_util.isDescriptor = function(name) { // eslint-disable-line
  return false;
};
