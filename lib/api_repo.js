...
function ApiRepo(opts) {
  ...
  if (!(this.repoDirs.length || this.zipUrl)) {
...
ApiRepo.prototype._buildProtos =
    function _buildProtos(name, version, language, done) {
      ...
      if (language === 'java') {
         ...
         * folder.
         * @param {string} protoPath the path of .proto file.
        ...
        var copyJavaPb = function copyJavaPb(repoDir, protoPath, next) {
          ...
          var dst = path.join(dstResourceDir, protoPath);
        ...
        var makeNodeModule = function makeNodeModule(err) {
          ...
          var fullPathProtos = []
          this._findProtos(
              name, version, Function.prototype, function (protoDir, proto) {
                fullPathProtos.push(path.join(protoDir, proto));
              }
          );

          var includePath = _.union(this.includePath, this.repoDirs);
 ...
 * and sets repoDirs to be an array containing that as its single element.
 *
 * if isGoogleApi is `true`, it confirms that each directory in repoDir has
 * 'google' subdirectory
 * @param {function(?Error)} done The callback.
 */
ApiRepo.prototype._checkRepo = function _checkRepo(done) {
  var that = this;
  if (this.repoDirs.length) {
    var checkDirs = this.repoDirs.map(newIsDirFunc);
    var checkGoogleDirs = []
    if (this.isGoogleApi) {
    ...
    async.waterfall(checkDirs.concat(checkGoogleDirs), function (err) {
      if (!err) {
        done(null);
      } else {
        done(err);
...
ApiRepo.prototype._findProtos = function _findProtos(name, version, done,
  ...
  if (this.isGoogleApi) {
    protoDirs = protoDirs.map(function (parts) {
      return path.join(parts, 'google');
    });
  }

  if (this.zipUrl) {
    protoDirs = protoDirs.map(function (parts) {
      return path.join(parts, name, version);
    });
  }
  ...
  async.waterfall(
      protoDirs.map(newIsDirFunc).concat(  // verify the proto dir exists
          [
 ...
 * @return {function(string, function(?Error))} A function to invoke protoc
 *   command.
 */
ApiRepo.prototype._makeProtocFunc = function _makeProtocFunc(opts, language) {
  ...
  // computeOutDir calculates the directory in which protoc should place the output.
  function computeOutDir(prefix, language, protoRepoDir, protoPath, done) {
    ...
    file.on('line', function (line) {
      ...
      if (match) {
        ...
        done(path.join(prefix, language, pkg));
      }
    });
  }
  ...
  function callProtoc(repoDir, protoPath, done) {
    computeOutDir(that.outDir, language, repoDir, protoPath, function (outDir) {
      callProtocWithOutDir(repoDir, protoPath, outDir, done);
    });
  }

  function callProtocWithOutDir(repoDir, protoPath, outDir, done) {
    ...
    if (language === 'go') {
      ...
      if (!opts.buildCommonProtos) {
        var pluginBin =
              that.depBins[that._getPluginName(language, that.overridePlugins)];
